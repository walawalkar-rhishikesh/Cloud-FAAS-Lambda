    ...
    getRecord(emailData, uidData)
        .then(data =>{
            ...
            if(data && UId){
                ...
                let difference = Date.now() - _Token;
                difference = difference / 1000

                if(difference <= 900){
                    console.log("_Token difference:::",difference )
                    return;
                }else{
...
function sendEmail(receiver, token, id, context,event ) {
    ...
    let reset_token = `https://prod.rhishikesh.me/reset?email:${receiver}&token=${token}&id=${id}`
